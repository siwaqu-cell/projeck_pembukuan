...
function savePembelian(data) {
  ...
  try {
    ...
    try {
      var itemNames = items.map(function(it){return tg(it.nama||'')+'x'+it.qty;}).join(', ');
      sendTelegramNotif('PEMBELIAN_PART', {id:id, items:itemNames, total:grandTotal, metodeBayar:data.metodeBayar, supplier:data.supplier, cabang:data.cabang});
...
function saveReturn(data) {
  ...
  try {
    ...
    // Telegram notification
    try {
      sendTelegramNotif('RETURN_SUPPLIER', {id:id, items:returnNames.join(', '), total:grandTotal, supplier:data.supplier, cabang:data.cabang});
    } catch(e) { Logger.log('TG notif: '+e.message); }
    lock.releaseLock();
...
function bayarHutang(data) {
  try {
    ...
    try { sendTelegramNotif('BAYAR_HUTANG', {id:data.idHutang, supplier:hutang.SUPPLIER, jumlah:bayar, sisa:sisa, cabang:hutang.CABANG, metodeBayar:data.metodeBayar}); } catch(e) {}
...
function bayarPiutang(data) {
  try {
    ...
    try { sendTelegramNotif('BAYAR_PIUTANG', {id:data.idPiutang, nama:pit.NAMA_PELANGGAN, jumlah:data.jumlah, cabang:pit.CABANG, metodeBayar:data.metodeBayar}); } catch(e) {}
