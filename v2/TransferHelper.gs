...
function saveTransfer(data, _token) {
  ...
  try {
    ...
    // Check stock availability — cocokkan supplier juga
    var stok = getStokByCabang(data.cabangAsal).filter(function(s) {
      return s.nama === data.namaPart && (!data.merkBarang || s.merkBarang === data.merkBarang) && (!data.supplier || s.supplier === data.supplier);
    });
    ...
    // Deduct from source — gunakan supplier yang tepat
    deductStock(data.cabangAsal, data.namaPart, data.merkBarang, data.subKategori, stok[0].supplier||'', qty);
    ...
    try { sendTelegramNotif('TRANSFER_CABANG', {items:data.namaPart+' x'+qty, cabangAsal:data.cabangAsal, cabangTujuan:data.cabangTujuan}); } catch(e) {}
...
function saveTransferMasal(data, _token) {
  var lock = LockService.getScriptLock();
  try {
    requireAuth(_token);
    lock.waitLock(30000);
    ...
    data.items.forEach(function(item, idx) {
      try {
        ...
        var stokList = getStokByCabang(data.cabangAsal).filter(function(s) {
          return s.nama === item.namaPart && (!item.merkBarang || s.merkBarang === item.merkBarang) && (!item.supplier || s.supplier === item.supplier);
        });
        ...
        // Deduct from source — gunakan supplier yang tepat
        deductStock(data.cabangAsal, item.namaPart, item.merkBarang, item.subKategori, stokList[0].supplier||'', qty);
    ...
    return {success:false, message:'Error sistem: '+safeError(e)};
  } finally {
    lock.releaseLock();
    // Telegram notification
    try { sendTelegramNotif('TRANSFER_CABANG', {items:totalSukses+' barang', cabangAsal:data.cabangAsal, cabangTujuan:data.cabangTujuan}); } catch(e2) {}
  }
}
